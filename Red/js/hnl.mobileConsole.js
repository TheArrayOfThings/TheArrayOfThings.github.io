...
if (typeof 'docReady' !== 'function') {
  (function (funcName, baseObj) {
    ...
    let i, len, readyList = [], readyFired = false, readyEventHandlersInstalled = false;
...
//define console letiable
let console = window.console;

let mobileConsole = (function () {
  'use strict';

  //options and other letiable containers
  let options = {
  ...
  let setLineStyle = (function () {
    ...
    toggleHeight = function () {
      if (status.initialized) {
        let existingPadding = parseInt(document.body.style.paddingBottom, 10) - Math.abs(elements.base.offsetHeight + elements.topbar.offsetHeight);
  ...
  function initConsoleElement() {
    ...
    function assemble() {
      let i = options.methods.length, key;
    ...
    function attach(console) {
      ...
      let existingPadding = isNaN(parseInt(document.body.style.paddingBottom, 10)) ? 0 : parseInt(document.body.style.paddingBottom, 10);
      setCSS(document.body, {
    ...
    function toggleLogType(e) {
      ...
      let elems = elements.lines[logType], i = elems.length;
    ...
    function setBinds() {
      let methods = options.methods, i = methods.length;
      while (i--) {
        if (methods[i] !== 'clear') {
    ...
    function init() {
      ...
      //expose Public methods and letiables
      return {
  ...
  function initConsole() {
    ...
    function urlFromString(string) {
      ...
      let match, uriPattern = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))/ig;
      try {
    ...
    function formatStackTrace(trace, origtrace) {
      ...
      let stackTraceOrig = (typeof trace !== 'undefined' && typeof trace[4] !== 'undefined') ? trace[4].stack : undefined;
    ...
    function colorizeData(key, value) {
      let valueColor =  '#3c53da', keyColor = '#ae33b7', classname = getClass(value);
    ...
    function toggleDetails(e) {
      let button = e.currentTarget || e.srcElement, i, hidden;
    ...
    function newConsole() {
      try {
        //get arguments, set lets
        let method = arguments[0], className, isHTMLElement,
        ...
        if (isRepeat(message, method) && method.indexOf('time') === -1) {
          ...
          switch (method) {
            ...
            case 'time':
            case 'timeEnd':
              let timerName = arguments[1].newMessage || 'default', now, passed;
              if (method === 'time') {
    ...
    function interceptConsole(method) {
      let original = console ? console[method] : missingMethod(), i, stackTraceOrig;
  ...
  function initCommandLine() {
    ...
    function getFromArrayById(id) {
      let pos = elements.acItems.map(function(x) {return x.id; }).indexOf(id);
      return {
        position: pos,
    ...
    function hoverAutoComplete(e) {
      ...
      let hovered = getFromArrayById('hover').element, target = e.target || e.srcElement, over;
    ...
    function toggleAutoComplete(show) {
      let hidden = (elements.autocomplete.currentStyle ? elements.autocomplete.currentStyle.display : window.getComputedStyle(elements.autocomplete, null).display) === 'none';
    ...
    function autoComplete(command) {
      ...
      let searchString = encodeURI(command), matches, match, row, i, maxAmount = options.browserinfo.isMobile ? 3 : 5;
